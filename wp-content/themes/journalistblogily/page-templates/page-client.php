...
<div id="primary" class="content-area archive">
        <main id="main" class="site-main" role="main">
            ...
            <header class="page-header page-header-client">
                <?php echo '<h1 class="page-title"><span class="page-title-pre">' . esc_html__( 'Client Page:', 'journalistblogily' ) . '</span>' . get_the_title() . '</h1>'; ?>
            </header>
	...
	<div id="primary-right" class="large-3 large-push-9 medium-4 medium-push-8 small-12 columns <?php if (!(have_comments() || comments_open())) : ?> no-comments-area<?php endif; ?>">
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					<div>
						...
						<div class="entry-content">
							<?php the_content(); ?>
							<?php wp_link_pages( array( 'before' => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'journalistblogily' ) . '</span>', 'after' => '</div>', 'link_before' => '<span>', 'link_after' => '</span>' ) ); ?>
                                                        <?php journalistblogily_jetpack_sharing(); ?>
                                                </div><!-- .entry-content -->
	
						<footer class="entry-footer">
							<?php edit_post_link( esc_html__( 'Edit', 'journalistblogily' ), '<span class="edit-link">', '</span>' ); ?>
						</footer><!-- .entry-meta -->
					</div>
				</article><!-- #post -->
			<?php endwhile; ?>
		...
		<?php journalistblogily_paging_nav(); ?>
	</div><!-- #primary-left -->
