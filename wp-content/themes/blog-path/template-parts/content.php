<?php
/**
 * Template part for displaying posts
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/
 *
 * @package Gist
 */
global $gist_theme_options;
$gist_entry_meta = $gist_theme_options['gist-blog-meta-options'];
$gist_show_content = $gist_theme_options['gist-blog-excerpt-options'];
$gist_show_featured_image = $gist_theme_options['gist-single-featured'];
$gist_read_more_text = $gist_theme_options['gist-blog-read-more-options'];
$gist_image_align = $gist_theme_options['gist-blog-featured-image'];
?>

<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <div class="p-15 <?php echo esc_attr($gist_image_align);?>">
        <div class="ct-featured-image">
            <?php
            if(! ( is_singular() ) ){
                gist_post_thumbnail();
            }
            else if( ( is_singular() ) && ( $gist_show_featured_image == 1 ) ) {
                gist_post_thumbnail();
            }
            ?>
        </div>
        <div class="entry-content">
            <header class="entry-header">
                <?php
                if (('post' === get_post_type()) && ($gist_entry_meta == 1)) : ?>
                <div class="entry-meta">
                    <?php gist_posted_on(); ?>
                </div><!-- .entry-meta -->
                <?php
            endif;
            if (is_singular()) :
                the_title('<h1 class="entry-title">', '</h1>');
            else :
                the_title('<h2 class="entry-title"><a href="' . esc_url(get_permalink()) . '" rel="bookmark">', '</a></h2>');
            endif;

            if (('post' === get_post_type()) && ($gist_entry_meta == 1)) : ?>
            <div class="entry-meta entry-category">
                <?php gist_entry_category(); ?>
            </div><!-- .entry-meta -->
            <?php
        endif;
        ?>
    </header>
    <!-- .entry-header -->
    <?php
    if (is_singular()) :
        the_content();
    else :
        if ( $gist_show_content == 'excerpt' ) {
            the_excerpt();
        } else {
            the_content();
        }
    endif;

    wp_link_pages(array(
        'before' => '<div class="page-links">' . esc_html__('Pages:', 'gist'),
        'after' => '</div>',
    ));
    ?>

    <footer class="entry-footer">
        <?php
        if ( ( !is_single() ) && ( $gist_show_content == 'excerpt' ) ) {
            if(!empty($gist_read_more_text)){                ?>
            <a href="<?php the_permalink(); ?>" class="btn btn-primary">
                <?php echo esc_html( $gist_read_more_text ); ?>

            </a>
            <?php
        }
    }
    gist_entry_footer();
    ?>
</footer>
<!-- .entry-footer -->
</div>
<!-- .entry-content -->
</div>
<!-- .p-15 -->


            <?php    
                /**
                 * gist_related_posts hook
                 * @since Gist 1.0.0
                 *
                 * @hooked gist_related_posts -  10
                 */
                if(! is_singular() ):
                    do_action( 'blog_path_page_related_posts' ,get_the_ID() );
                endif;
            ?>

</article><!-- #post-<?php the_ID(); ?> -->
