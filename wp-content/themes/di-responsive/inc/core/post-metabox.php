...
// Do if is admin only.
if( is_admin() ) {
    add_action( 'load-post.php', 'di_responsive_post_meta_box_first_func' );
    add_action( 'load-post-new.php', 'di_responsive_post_meta_box_first_func' );
}

/**
 * Calls the class on the post edit screen.
 */
function di_responsive_post_meta_box_first_func() {
    new Di_Responsive_Post_Meta_Box_Main_Class();
}
 
/**
 * The Class.
 */
class Di_Responsive_Post_Meta_Box_Main_Class {
    ...
    public function add_meta_box( $post_type ) {
        if ( $post_type == 'post' ) {
            add_meta_box(
                'di_responsive_post_meta_box_cntnr',
                __( 'Di Responsive Theme Options for this Post', 'di-responsive' ),
    ...
    public function render_meta_box_content( $post ) {
        ...
        wp_nonce_field( 'di_responsive_post_meta_bx_key', 'di_responsive_post_meta_bx_key_nonce' );
        ...
        <?php
        do_action( 'di_responsive_post_metabox_render', $post );
    }
    ...
    public function save( $post_id ) {
        ...
        if( ! isset( $_POST['di_responsive_post_meta_bx_key_nonce'] ) ) {
            return $post_id;
        }
 
        $nonce = wp_unslash( $_POST['di_responsive_post_meta_bx_key_nonce'] );
 
        // Verify that the nonce is valid.
        if( ! wp_verify_nonce( $nonce, 'di_responsive_post_meta_bx_key' ) ) {
            return $post_id;
        }
        ...
        do_action( 'di_responsive_post_metabox_save', $post_id );
