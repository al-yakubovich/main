<?php
/**
 * Primary class. This sets up and runs the show.
 *
 * @package    ActionsMB
...
if ( ! class_exists( 'ActionsMB' ) ) {

	/**
	 * Main ActionsMB class.  Runs the show.
	 *
	 * @since  1.0.0
	 * @access public
	 */
	final class ActionsMB {
		...
		private function setup() {
			...
			$this->dir_path  = apply_filters( 'actionsmb_dir_path', trailingslashit( get_template_directory() ) );
			$this->dir_uri   = apply_filters( 'actionsmb_dir_uri',  trailingslashit( get_template_directory_uri() ) );

			$this->tmpl_path = trailingslashit( $this->dir_path . 'inc/tmpl' );
		}
		...
		private function includes() {
			...
			// Load base classes.
			require_once( $this->dir_path . 'inc/manager/class-manager.php' );
			require_once( $this->dir_path . 'inc/manager/class-section.php' );
			require_once( $this->dir_path . 'inc/manager/class-control.php' );
			require_once( $this->dir_path . 'inc/manager/class-setting.php' );

			// Load control sub-classes.
			require_once( $this->dir_path . 'inc/manager/controls/class-control-checkboxes.php'    );
			require_once( $this->dir_path . 'inc/manager/controls/class-control-radio.php'         );
			require_once( $this->dir_path . 'inc/manager/controls/class-control-radio-image.php'   );

			require_once( $this->dir_path . 'inc/manager/controls/class-control-parent.php'        );

			// Load setting sub-classes.
			require_once( $this->dir_path . 'inc/manager/settings/class-setting-multiple.php' );
			require_once( $this->dir_path . 'inc/manager/settings/class-setting-array.php'    );

			// Load functions.
			require_once( $this->dir_path . 'inc/manager/functions-core.php' );
		}
		...
		private function setup_actions() {
			...
			add_action( 'actionsmb_register', array( $this, 'register_manager_types' ), -95 );
			add_action( 'actionsmb_register', array( $this, 'register_section_types' ), -95 );
			add_action( 'actionsmb_register', array( $this, 'register_control_types' ), -95 );
			add_action( 'actionsmb_register', array( $this, 'register_setting_types' ), -95 );
		}

		/**
		 * Registration callback. Fires the `actionsmb_register` action hook to
		...
		public function register() {
			...
			do_action( 'actionsmb_register', $this, $post_type );
			...
			add_action( 'actionsmb_enqueue_scripts', array( $this, 'enqueue'         ) );
		...
		/**
		 * Registers a manager type.  This is just a method of telling ActionsMB
		...
		/**
		 * Registers a section type.  This is just a method of telling ActionsMB
		...
		/**
		 * Registers a control type.  This is just a method of telling ActionsMB
		...
		/**
		 * Registers a setting type.  This is just a method of telling ActionsMB
		...
		public function register_manager_types() {
			...
			$this->register_manager_type( 'default', 'ActionsMB_Manager' );
		}
		...
		public function register_section_types() {
			...
			$this->register_section_type( 'default', 'ActionsMB_Section' );
		}
		...
		public function register_control_types() {
			...
			$this->register_control_type( 'default',       'ActionsMB_Control'               );
			$this->register_control_type( 'checkboxes',    'ActionsMB_Control_Checkboxes'    );
			$this->register_control_type( 'color',         'ActionsMB_Control_Color'         );
			$this->register_control_type( 'datetime',      'ActionsMB_Control_Datetime'      );
			$this->register_control_type( 'excerpt',       'ActionsMB_Control_Excerpt'       );
			$this->register_control_type( 'image',         'ActionsMB_Control_Image'         );
			$this->register_control_type( 'palette',       'ActionsMB_Control_Palette'       );
			$this->register_control_type( 'radio',         'ActionsMB_Control_Radio'         );
			$this->register_control_type( 'radio-image',   'ActionsMB_Control_Radio_Image'   );
			$this->register_control_type( 'select-group',  'ActionsMB_Control_Select_Group'  );
			$this->register_control_type( 'textarea',      'ActionsMB_Control_Textarea'      );
			$this->register_control_type( 'multi-avatars', 'ActionsMB_Control_Multi_Avatars' );
			$this->register_control_type( 'parent',        'ActionsMB_Control_Parent'        );
		}
		...
		public function register_setting_types() {
			...
			$this->register_setting_type( 'default',  'ActionsMB_Setting'          );
			$this->register_setting_type( 'single',   'ActionsMB_Setting'          );
			$this->register_setting_type( 'multiple', 'ActionsMB_Setting_Multiple' );
			$this->register_setting_type( 'array',    'ActionsMB_Setting_Array'    );
			$this->register_setting_type( 'datetime', 'ActionsMB_Setting_Datetime' );
		}
		...
		public function enqueue_scripts() {
			...
			do_action( 'actionsmb_enqueue_scripts' );
		}
		...
		public function enqueue() {
			...
			wp_enqueue_script( 'actionsmb', $this->dir_uri . "assets/js/actionsmb{$min}.js", array( 'backbone', 'wp-util' ), '', true );

			// Enqueue the main plugin style.
			wp_enqueue_style( 'actionsmb', $this->dir_uri . "assets/css/actionsmb{$min}.css" );
		...
		public function meta_box( $post, $metabox ) {
			...
			wp_nonce_field( "actionsmb_{$manager->name}_nonce", "actionsmb_{$manager->name}" );
		}
		...
		public function localize_scripts() {
			...
			wp_localize_script( 'actionsmb', 'actionsmb_data', $json );
		}
		...
		public function print_templates() {
			...
			<script type="text/html" id="tmpl-actionsmb-nav">
				<?php actionsmb_get_nav_template(); ?>
			</script>
	...
	/**
	 * Gets the instance of the `ActionsMB` class.  This function is useful for quickly grabbing data
