window.actionsmb = window.actionsmb || {};

( function() {

	// Bail if we don't have the JSON, which is passed in via `wp_localize_script()`.
	if ( _.isUndefined( actionsmb_data ) ) {
		return;
	}

	/**
	 * Our global object.  The `actionsmb` object is just a wrapper to house everything
	...
	api.templates.register_manager = function( type ) {
		...
		this.managers[ type ] = wp.template( 'actionsmb-manager-' + type );
	};
	...
	api.templates.register_section = function( type ) {
		...
		this.sections[ type ] = wp.template( 'actionsmb-section-' + type );
	};
	...
	api.templates.register_control = function( type ) {
		...
		this.controls[ type ] = wp.template( 'actionsmb-control-' + type );
	};
	...
	api.render = function() {
		...
		_.each( actionsmb_data.managers, function( data ) {
			...
			var metabox = document.getElementById( 'actionsmb-ui-' + manager.get( 'name' ) );

			// Add the `.actionsmb-ui` class to the meta box.
			metabox.className += ' actionsmb-ui';
	...
	// Nav template.
	var nav_template = wp.template( 'actionsmb-nav' );
	 ...
	 * `actionsmb.views.register_manager()` function.
	 *
	 * @since  1.0.0
	 * @access public
	 * @var    object
	...
	api.views.managers[ 'default' ] = Backbone.View.extend( {
		...
		attributes : function() {
			return {
				'id'    : 'actionsmb-manager-' + this.model.get( 'name' ),
				'class' : 'actionsmb-manager actionsmb-manager-' + this.model.get( 'type' )
			};
		},
		...
		subview_render : function() {
			...
			sections.forEach( function( section, i ) {
				...
				document.querySelector( '#actionsmb-ui-' + section.get( 'manager' ) + ' .actionsmb-nav'     ).appendChild( nav_view.render().el     );
				...
				document.querySelector( '#actionsmb-ui-' + section.get( 'manager' ) + ' .actionsmb-content' ).appendChild( view.render().el );
			...
			_.each( this.model.get( 'controls' ), function( data ) {
				...
				// Render the view.
				document.getElementById( 'actionsmb-' + control.get( 'manager' ) + '-section-' + control.get( 'section' ) ).appendChild( view.render().el );
	 ...
	 * `actionsmb.views.register_section()` function.
	 *
	 * @since  1.0.0
	 * @access public
	 * @var    object
	...
	api.views.sections[ 'default' ] = Backbone.View.extend( {
		...
		attributes : function() {
			return {
				'id'          : 'actionsmb-' + this.model.get( 'manager' ) + '-section-' + this.model.get( 'name' ),
				'class'       : 'actionsmb-section actionsmb-section-' + this.model.get( 'type' ),
	 ...
	 * `actionsmb.views.register_control()` function.
	 *
	 * @since  1.0.0
	 * @access public
	 * @var    object
	...
	api.views.controls[ 'default' ] = Backbone.View.extend( {
		...
		attributes : function() {
			return {
				'id'    : 'actionsmb-control-' + this.model.get( 'name' ),
				'class' : 'actionsmb-control actionsmb-control-' + this.model.get( 'type' )
			};
		},
	...
	api.views.register_control( 'color', {
		...
		ready : function() {
			...
			jQuery( this.$el ).find( '.actionsmb-color-picker' ).wpColorPicker( options );
		}
	} );
	...
	api.views.register_control( 'image', {
		...
		// Adds custom events.
		events : {
			'click .actionsmb-add-media'    : 'showmodal',
			'click .actionsmb-change-media' : 'showmodal',
			'click .actionsmb-remove-media' : 'removemedia'
		},
