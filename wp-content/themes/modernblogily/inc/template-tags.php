...
if ( ! function_exists( 'modernblogily_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function modernblogily_posted_on() {
        global $post;
    ...
    $byline = sprintf(
        ...
        esc_html_x( 'By %s', 'post author', 'modernblogily' ),
    ...
    //echo '<span class="byline"> ' . $byline . '</span>';
        ...
        /* translators: used between list items, there is a space after the comma */
                if( 'post' === get_post_type() ) {
                    $categories_list = get_the_category_list( __( '</li><li>', 'modernblogily' ) );
...
if ( ! function_exists( 'modernblogily_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function modernblogily_entry_footer() {
    global $post;
    ...
    if ( 'post' === get_post_type() || 'jetpack-portfolio' === get_post_type() ) {
        /* translators: used between list items, there is a space after the comma */
            if( 'post' === get_post_type() ) {
        $categories_list = get_the_category_list( esc_html__( ', ', 'modernblogily' ) );
            } elseif( 'jetpack-portfolio' === get_post_type() ) {
                $categories_list = get_the_term_list( $post->ID, 'jetpack-portfolio-type', '', esc_html_x(', ', 'Used between list items, there is a space after the comma.', 'modernblogily' ), '');
            }
        if ( $categories_list && modernblogily_categorized_blog() ) {
    ...
    if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
        ...
        comments_popup_link( esc_html__( 'Leave a comment', 'modernblogily' ), esc_html__( '1 Comment', 'modernblogily' ), esc_html__( '% Comments', 'modernblogily' ) );
        echo '</span>';
    }
}
endif;
 ...
 * @return bool
 */
function modernblogily_categorized_blog() {
    if ( false === ( $all_the_cool_cats = get_transient( 'modernblogily_categories' ) ) ) {
        ...
        set_transient( 'modernblogily_categories', $all_the_cool_cats );
    }

    if ( $all_the_cool_cats > 1 ) {
        // This blog has more than 1 category so modernblogily_categorized_blog should return true.
        return true;
    } else {
        // This blog has only 1 category so modernblogily_categorized_blog should return false.
        return false;
    }
}

/**
 * Flush out the transients used in modernblogily_categorized_blog.
 */
function modernblogily_category_transient_flusher() {
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
        return;
    }
    // Like, beat it. Dig?
    delete_transient( 'modernblogily_categories' );
}
add_action( 'edit_category', 'modernblogily_category_transient_flusher' );
add_action( 'save_post',     'modernblogily_category_transient_flusher' );



/*==============================================================================
 * modernblogily CUSTOM TAGS BELOW
 =============================================================================*/
/**
 * Fancy excerpts
 * 
 ...
 */
function modernblogily_fancy_excerpt() {
    global $post;
    if( is_archive() ) {
       the_excerpt();
        echo '<div class="continue-reading">';
        echo '<a class="more-link more-link-activated" href="' . esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue Reading ', 'modernblogily' ) . get_the_title() . '" rel="bookmark">' . esc_html__( 'Continue Reading', 'modernblogily' ) . '</a>'; 
        echo '</div>';
    } elseif ( is_page_template( 'page-templates/page-child-pages.php' ) ) {
        the_excerpt();
         echo '<div class="continue-reading">';
        echo '<a class="continue-reading-arrow" href="' . esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue Reading ', 'modernblogily' ) . get_the_title() . '" rel="bookmark">&rarr;</a>'; 
        echo '</div>';
 ...
 */
function modernblogily_excerpt_more( $more ) {
    if( is_admin() ) {
        return $more;
    }
    return " &hellip;";
}
add_filter( 'excerpt_more', 'modernblogily_excerpt_more' );

/**
 * Add an author box below posts
 * @link http://www.wpbeginner.com/wp-tutorials/how-to-add-an-author-info-box-in-wordpress-posts/
 */
function modernblogily_author_box() {
    global $post;
    ...
    if ( isset( $post->post_author ) ) {
        ...
        $author_details .= '<h3 class="author-title"><span>' . esc_html__( 'About ', 'modernblogily' );
            if ( is_author() ) $author_details .= $display_name;        // If an author archive, just show the author name
            else $author_details .= esc_html__( 'the Author', 'modernblogily' ); // If a regular page, show "About the Author"
        ...
        if ( ! is_author() ) {  // Don't show the meta info on an author archive page
            $author_details .= '<p class="author-links entry-meta"><span class="vcard">' . esc_html__( 'All posts by', 'modernblogily' ) . '<a class="fn" href="' . esc_url( $user_posts ) . '">' . $display_name . '</a></span>';

            // Check if author has a website in their profile
            if ( ! empty( $user_site ) ) 
                $author_details .= '<a class="author-site" href="' . esc_url( $user_site ) . '" target="_blank" rel="nofollow">' . esc_html__( 'Website', 'modernblogily' ) . '</a></p>';
            else $author_details .= '</p>';
        }
        
        $author_details .= '</section>';
        $author_details .= '</div>';
        $author_details .= '<p class="show-hide-author label">' . esc_html__( 'Hide', 'modernblogily' ) . '</p>';
...
function modernblogily_portfolio_index_footer() {
    $time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
    if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
            $time_string = '<i class="fa fa-refresh tip"><span class="tooltip">' .
                        esc_attr__( 'First posted: ', 'modernblogily' ) .
    ...
    $project_type = get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '<span class="portfolio-entry-meta cat-links">', esc_html_x( ', ', 'Used between list items, there is a space after the comma.', 'modernblogily' ), '</span>' );
...
function modernblogily_breadcrumbs() {
    ...
    $home_link = '<a aria-label="' . esc_html__( 'Home', 'modernblogily' ) .'" title="' . esc_html__( 'Home', 'modernblogily' ) .'" class="breadcrumb-home" href="' . esc_url( home_url() ) . '"><i class="fa fa-home"></i></a>' . $separator;
    
    $output .= "<div aria-label='" . esc_html__( 'You are here:', 'modernblogily' ) . "' id='$breadcrumb_id' class='$breadcrumb_class'>";
    $output .= $home_link;
 ...
 * Social Menu
 */
function modernblogily_social_menu() {
 ...
 */
function modernblogily_post_icon() {
...
function modernblogily_the_post_icon() {
    echo wp_kses( modernblogily_post_icon(), array( 'span' => array( 'class' => array() ) ) );
}

/**
 * Function to show the Jetpack sharing and Likes only at the designated locations in Posts and Pages
 */
function modernblogily_jetpack_sharing() {
 ...
 */
function modernblogily_post_navigation() {
    ...
    <nav class="navigation post-navigation" role="navigation">
        <p class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'modernblogily' ); ?></p>
        <div class="nav-links" data-equalizer>
                <?php
                        previous_post_link( '<div class="nav-previous" data-equalizer-watch><div class="nav-indicator">' . esc_html_x( 'Previous Post:', 'Previous post', 'modernblogily' ) . '</div><h4>%link</h4></div>', '%title' );
                        next_post_link(     '<div class="nav-next" data-equalizer-watch><div class="nav-indicator">'     . esc_html_x( 'Next Post:', 'Next post', 'modernblogily' ) . '</div><h4>%link</h4></div>', '%title' );
                ?>
        </div> <!-- .nav-links -->
    </nav> <!-- .navigation -->
    <?php
...
if ( ! function_exists( 'modernblogily_paging_nav' ) ) :
/**
 ...
 */
function modernblogily_paging_nav() {
        the_posts_pagination( array(               
            'mid_size'      => 3,
            'prev_text' => __( 'Previous', 'modernblogily' ),
            'next_text' => __( 'Next', 'modernblogily' ),
            'type'      => 'list',
        ));
}
endif;
...
if ( ! function_exists( 'modernblogily_copyright' ) ) :
/** 
 ...
 */
function modernblogily_copyright() {
