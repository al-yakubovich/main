 ...
 */
function modernblogily_jetpack_setup() {
	// Add theme support for Infinite Scroll.
	add_theme_support( 'infinite-scroll', array(
		'footer_widgets'=> 'sidebar-1',
                'container'     => 'post-archives',
		'render'	=> 'modernblogily_infinite_scroll_render',
		'footer'	=> 'colophon',
                'post_per_page' => 8,
	) );
	...
	add_image_size( 'modernblogily-logo', 200, 200 );
	add_theme_support( 'site-logo', array( 'size' => 'modernblogily-logo' ) );
 ...
 */
function modernblogily_jetpack_default_hide_cf( $hidden, $screen ) {
...
add_action( 'default_hidden_meta_boxes', 'modernblogily_jetpack_default_hide_cf', 10, 2 );

/**
 * Custom render function for Infinite Scroll.
 */
function modernblogily_infinite_scroll_render() {
	while ( have_posts() ) {
		the_post();
		if ( is_search() ) :
 ...
 */
function modernblogily_the_site_logo() {
...
if( !function_exists( 'modernblogily_social_menu' ) ) :
 ...
 */
global $modernblogily_current_class;
$modernblogily_current_class = 'odd';
function modernblogily_odd_even_post_class( $classes ) {
    if( get_post_type( get_the_ID() ) === 'jetpack-testimonial' ) {
        global $modernblogily_current_class;
        $classes[] = $modernblogily_current_class;
        $modernblogily_current_class = ( $modernblogily_current_class == 'odd' ) ? 'even' : 'odd';
    }
    return $classes;
}
add_filter( 'post_class', 'modernblogily_odd_even_post_class' );

/**
 * Move JetPack Share and Like buttons
 * @link https://jetpack.com/2013/06/10/moving-sharing-icons/
 */
function modernblogily_jptweak_remove_share() {
...
add_action( 'loop_start', 'modernblogily_jptweak_remove_share' );